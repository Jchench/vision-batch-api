...
process_documents <- function(doc_path_A, doc_path_B, law_name) {
  ...
  # Save the results
  accuracy_embedd_og <- tibble(
    law = law_name,
    machine_readable = proportionA_not_in_B,
    embed = proportionB_not_in_A
  )
  
  assign(paste0(law_name, "_accuracy"), accuracy_embedd_og, envir = .GlobalEnv)
  
  save(accuracy_embedd_og, file = paste0("results/", law_name, "_accuracy.rda"))
...
document_pairs <- list(
  list("machine_readble_1/PL107_204.txt", "extracted_text/extracted_PL107_204.txt", "PL107_204")
)
